...
object corefunctions1 {
  ...
  println("Number of partition of the filtered file" + rddexerjump.getNumPartitions); 
  ...
  println(s"No of lines that does not contain Credit: $cnt")
   ...
   // filter only california customer who did cash trans and show me the sum of amount 
  ...
  rddkvpair.reduceByKey((a,b)=> (mintrans(a,b))).take(10).foreach(println)
   ...
   println("Brodcast real example")
      ...
      //rdd (4) val kvpair= r.map(split).map(x=>toMap(x(3)->x(4))).collect
