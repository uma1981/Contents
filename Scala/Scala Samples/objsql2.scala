...
.option("compression","gzip").partitionBy("dt").csv("hdfs://localhost:54310/user/hduser/csvdataout")
...
.option("allowSingleQuotes","true").option("allowNumericLeadingZeros","true").option("mode","failfast")
...
spark.sql("select * from default.customermysql").show(5,false)
