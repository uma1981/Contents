	...
	def main(args: Array[String]) {
		...
		if (scdf1g == 1) {
			val spark = SparkSession.builder().appName("Sample sql app").master("local[*]").getOrCreate()
					...
					.option("header", "true")
					.schema(schema)
					.csv("C:\\Users\\BhagyaGanisetti\\Downloads\\scd\\scd1\\histdata1.csv")
					val dfnew1 = spark.read.option("inferschema", "true")
					.option("delimiter", ",")
					.option("header", "true")
					.schema(schema)
					.csv("C:\\Users\\BhagyaGanisetti\\Downloads\\scd\\scd1\\newdata1.csv")
						...
						if (!dfsrc1.rdd.isEmpty()) {
						 ...
						 newdftest.show
							if (!dfnew1.rdd.isEmpty()) {
								...
								newdf.count
								newdf.show
								newdf.coalesce(1).write.mode("overwrite").csv("C:\\Users\\BhagyaGanisetti\\Downloads\\scd\\scd1\\histdata1.csv")
								
							}
						} else {
							println("No Historical data to process for scd1 , loading as history")
							dfnew1.coalesce(1).write.mode("overwrite").csv("C:\\Users\\BhagyaGanisetti\\Downloads\\scd\\scd1\\histdata1.csv")
						}
					} catch {
